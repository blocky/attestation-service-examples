...
- The function uses `as.Log` to write a message to the Blocky AS server's log,
  which is useful for debugging and monitoring your function's behavior.
...
where we use `cat` to read the function call from 
[`fn-call.json`](./fn-call.json), pipe it to
`bky-as attest-fn-call`, and save the output to `tmp/out.json`.
If you inspect [`fn-call.json`](./fn-call.json), you'll see:
