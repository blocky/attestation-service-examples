...
We'll implement the oracle as `pointsLeaderNBA` in
...
Notice the `input` section, which contains the parameters for `pointsLeaderNBA`,
...
Next, we define the `pointsLeaderNBA` function in [`main.go`](./main.go):
...
returning the pointer from `pointsLeaderNBA` to the Blocky AS server host runtime.

### Step 2: Make a request to the CoinGecko API

The `getPriceFromCoinGecko` function, in `pointsLeaderNBA`, will make an HTTP request
...
specified `market` and returns the price as a `Price` struct to the `pointsLeaderNBA`
function. The `pointsLeaderNBA` function returns a `Result` containing the `Price` to
the Blocky AS server to create an attestation over the function call and the
`Result` struct.

### Step 3: Run the oracle

To run `pointsLeaderNBA`, you need call:
...
`pointsLeaderNBA`, or changing out the API endpoint in `getPriceFromCoinGecko` to fetch data from
